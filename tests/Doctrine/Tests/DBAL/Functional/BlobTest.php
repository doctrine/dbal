...
class BlobTest extends \Doctrine\Tests\DbalFunctionalTestCase
{
    protected function setUp()
    {
        ...
        if ($this->conn->getDriver() instanceof \Doctrine\DBAL\Driver\PDOSqlsrv\Driver) {
        ...
        $this->conn->exec($this->conn->getDatabasePlatform()->getTruncateTableSQL('blob_table'));
    }

    public function testInsert()
    {
        $ret = $this->conn->insert('blob_table',
    ...
    {
        $rows = $this->conn->fetchAll('SELECT * FROM blob_table');

        self::assertCount(1, $rows);
        $row = array_change_key_case($rows[0], CASE_LOWER);

        $blobValue = Type::getType('binary')->convertToPHPValue($row['binaryfield'], $this->conn->getDatabasePlatform());
    ...
    private function assertBlobContains($text)
    {
        ...
        $blobValue = Type::getType('blob')->convertToPHPValue($row['blobfield'], $this->conn->getDatabasePlatform());
