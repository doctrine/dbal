...
class DBAL630Test extends \Doctrine\Tests\DbalFunctionalTestCase
{
    ...
    protected function setUp()
    {
        ...
        try {
            $this->conn->exec('CREATE TABLE dbal630 (id SERIAL, bool_col BOOLEAN NOT NULL);');
            $this->conn->exec('CREATE TABLE dbal630_allow_nulls (id SERIAL, bool_col BOOLEAN);');
    ...
    {
        if ($this->running) {
            $this->conn->getWrappedConnection()->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
        }
    ...
    {
        $this->conn->executeUpdate('INSERT INTO dbal630 (bool_col) VALUES(false)');
        $id = $this->conn->lastInsertId('dbal630_id_seq');
        self::assertNotEmpty($id);

        $row = $this->conn->fetchAssoc('SELECT bool_col FROM dbal630 WHERE id = ?', array($id));
    ...
    {
        $this->conn->getWrappedConnection()->setAttribute(PDO::ATTR_EMULATE_PREPARES, true);

        $platform = $this->conn->getDatabasePlatform();

        $stmt = $this->conn->prepare('INSERT INTO dbal630 (bool_col) VALUES(?)');
    ...
    public function testBooleanConversionNullParamEmulatedPrepares(
        ...
        $stmt = $this->conn->prepare('INSERT INTO dbal630_allow_nulls (bool_col) VALUES(?)');
        $stmt->bindValue(1, $platform->convertBooleansToDatabaseValue($statementValue));
        $stmt->execute();

        $id = $this->conn->lastInsertId('dbal630_allow_nulls_id_seq');

        self::assertNotEmpty($id);

        $row = $this->conn->fetchAssoc('SELECT bool_col FROM dbal630_allow_nulls WHERE id = ?', array($id));
