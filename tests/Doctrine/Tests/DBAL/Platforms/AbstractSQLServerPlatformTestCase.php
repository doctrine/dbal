...
abstract class AbstractSQLServerPlatformTestCase extends AbstractPlatformTestCase
{
    ...
    public function testModifyLimitQueryWithSubSelectContainingAggregate()
    {
    	...
    	$expectedQuery 						= 'SELECT * FROM (SELECT son.label AS Name, ROW_NUMBER() OVER (ORDER BY identifier DESC) AS doctrine_rownum FROM SqlObjectName son WHERE ( SELECT COUNT(eso.identifier) FROM ExtractedSqlObject eso INNER JOIN ProductionDbName pdn ON eso.ref_ProductionDbName_ID = pdn.identifier AND (pdn.label IN (?, ?)) WHERE eso.ref_SqlObjectName_ID = son.identifier) > 0) AS doctrine_tbl WHERE doctrine_rownum BETWEEN 1 AND 1 ORDER BY doctrine_rownum';
    	//$beforePatchFaultyResult 	= 'SELECT * FROM (SELECT son.label AS Name FROM SqlObjectName son WHERE ( SELECT COUNT(eso.identifier), ROW_NUMBER() OVER (ORDER BY identifier DESC) AS doctrine_rownum FROM ExtractedSqlObject eso INNER JOIN ProductionDbName pdn ON eso.ref_ProductionDbName_ID = pdn.identifier AND (pdn.label IN (?, ?)) WHERE eso.ref_SqlObjectName_ID = son.identifier) > 0) AS doctrine_tbl WHERE doctrine_rownum BETWEEN 1 AND 1 ORDER BY doctrine_rownum
