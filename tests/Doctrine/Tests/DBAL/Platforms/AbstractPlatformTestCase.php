...
abstract class AbstractPlatformTestCase extends \Doctrine\Tests\DbalTestCase
{
    ...
    public function testAlterTableRenameIndex()
    {
        ...
        $tableDiff->renamedIndexes = array(
            '`idx-foo`' => new Index('`idx-bar`', array('id'))
        );// Index name with special character in name (needs quotation on some platforms, e.g. Sqlite).
    ...
    protected function getAlterTableRenameIndexInSchemaSQL()
    {
        return array(
            'DROP INDEX `idx-foo`',
            'CREATE INDEX `idx-bar` ON myschema.mytable (id)',
        );// Index name with special character in name (needs quotation on some platforms, e.g. Sqlite).
    }
