...
result, classes extending them must be declared as read-only as well.
...
2. The following keys of the above `$parameters` parameter are no longer optional: `indexes`, `uniqueConstraints`,
   `foreignKeys`.
